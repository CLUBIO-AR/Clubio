...
export async function GET(request: Request) {
  ...
  if (!licencias?.length) {
    await logCron({ tipo: "generar_cuotas", esDispatcher: true, gymsTotal: 0, gymsOk: 0, gymsError: 0, duracionMs: Date.now() - startTime });
  ...
  console.log(`[cron:generar-cuotas:dispatcher] dispatched=${licencias.length} ok=${succeeded} fail=${failed}`);
  await logCron({ tipo: "generar_cuotas", esDispatcher: true, gymsTotal: licencias.length, gymsOk: succeeded, gymsError: failed, duracionMs: Date.now() - startTime });
